...
{
    id: String
    // other fields arent important here
}
...
pub fn make_pokemon_enum(input: TokenStream) -> TokenStream
{
    ...
    let (ids_strings, paths): (Vec<String>, Vec<String>) = WalkDir::new(&target_dir)
            ...
            .filter_map(|entry| {
                ...
                if !(path.is_file() && path.extension()? == FILE_FORMAT) { return None }
                Some(path)
            })
            .fold(
                || (vec![], vec![]),
                |(mut names, mut paths), path| {
                    let content = fs::read_to_string(&path).unwrap_or_else(|err| panic!("Failed to read file at {}: {}", path.display(), err));
                    ...
                    paths.push(path.into_os_string().into_string().expect("Failed to convert OsString into string"));
                    (names, paths)
                }
            )
            .reduce(
    ...
    if ids.is_empty() { panic!("Couldn't find any pokemon config files in {}", target_dir.display()) }
