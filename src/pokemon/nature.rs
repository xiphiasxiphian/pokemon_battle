...
impl Nature
{
    ...
    {
        if stat == Stat::Health { return 1.0 } // this is kinda an error case, but 1.0 just signifies "do nothing"

        let (inc, dec) = self.stat_changes();
        if inc == dec { 1.0 }
        else if stat == inc { Self::NATURE_UP }
        else if stat == dec { Self::NATURE_DOWN }
        else { 1.0 }
    }
}
